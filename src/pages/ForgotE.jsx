...
export const ForgotE = () => {
  const { register, handleSubmit, formState: { errors } } = useForm()
  const { fetchDataBackend } = useFetch()

  const sendMail = async (data) => {
    const url = `${import.meta.env.VITE_BACKEND_URL}/recuperar-password-e`
      ...
      <div className="w-full sm:w-1/2 h-screen bg-white flex justify-center items-center">
        <div className="md:w-4/5 sm:w-full">
          ...
          <div className="tracking-wider flex justify-between items-center mt-2 text-sm text-gray-700">
            <Link to="/login-estudiante" className="underline hover:text-black tracking-widest">Regresar</Link>
          </div>
        </div>
      </div>
    </motion.div>
